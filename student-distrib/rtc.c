...
// volatile uint32_t rtc_clap_counter[3]={0,0,0};
...
void rtc_init(void) {
    ...
    for(i=0;i<TERMINAL_NUMBER;i++){   //initialize all the counter and tick
       rtc_num_tick_virtual[i]=0;
       rtc_interrupt_occur_virtual[i]=0;
       rtc_clap_counter_virtual[i]=0;
    }
...
void rtc_interrupt_handler(void) {
    ...
    if(rtc_clap_counter_virtual[current_running_idx]>=terminals[current_running_idx].clap){   //a virtual interrupt happens
        rtc_interrupt_occur_virtual[current_running_idx]=1;
        rtc_clap_counter_virtual[current_running_idx]=0;            //reset counter
    }
    else{
        int i ;
        for(i=0; i< TERMINAL_NUMBER;i++){
          rtc_clap_counter_virtual[i]++;          //increment all counters
        }
        // rtc_clap_counter_virtual[current_running_idx]++;
    }
    rtc_num_tick_virtual[current_running_idx]++;    //increment tick
...
#ifdef VIRTUAL_RTC
    //set the frqency to 1024
    uint8_t rightrate= (uint8_t)(6);    //rate is freq index+6, index here is 0;
    outb(DISABLE_NMI+REGISTER_A, RTC_PORT);
    rightrate = rightrate & 0x0F ;      //set REGISTER_A value
    uint8_t readrate=inb(CMOS_PORT);
    if(readrate!=rightrate){
        outb(rightrate,CMOS_PORT);
    }

    rtc_reset();            //clear tick and counter
...
int32_t rtc_read(int32_t fd, void* buf, int32_t nbytes){
    ...
    while(!rtc_interrupt_occur_virtual[current_running_idx]){
        //KEEP SPIN AND WAIT
    }
    ...
    rtc_interrupt_occur_virtual[current_running_idx]=0;
...
int32_t rtc_write(int32_t fd ,const void* buf, int32_t nbytes){
    ...
    terminals[current_running_idx].freq=(*(int32_t*)buf);       //write freq into current terminal
...
int32_t rtc_set_freq(int32_t freq){
    ...
    terminals[current_running_idx].clap= MAX_FREQ/freq-1;
    return 0;
...
void rtc_reset(){
    ...
    rtc_num_tick_virtual[current_running_idx]=0;        //clear
    rtc_clap_counter_virtual[current_running_idx]=0;      //clear
    return;
