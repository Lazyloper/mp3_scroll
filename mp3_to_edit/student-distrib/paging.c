...
int32_t scheduler_vidmem_remap(uint32_t terminal_num, int32_t vidmap_flag) {
		...
		//PTE for video memory
		page_tbl[(VIDEO_MEM_BUF & MASK_TABLE) >> 12].page_base_addr = (video_physical) >> 12;
		...
		flush_tlb();
		if (terminal_num == 1){
			*(uint8_t *)(VIDEO_MEM_BUF) = 0x7e;
			*(uint8_t *)(VIDEO_MEM_BUF + 1) = 0x07;
		} else if (terminal_num == 2) {
			*(uint8_t *)(VIDEO_MEM_BUF) = 0x40;
			*(uint8_t *)(VIDEO_MEM_BUF + 1) = 0x07;
		}
		return 0;
}
